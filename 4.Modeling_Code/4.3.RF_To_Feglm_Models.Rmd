...
```{r}
data <- readRDS("/Volumes/Seagate Portable Drive/Relative_Humidity_Project/5.modeling_data/5.2.RF_predicted_data_with_PSW_Color.rds")
...
predictions_12AM <- predict(get(model_12AM), newdata = hour1)
...
```{r}
feglm_predict_data <- cbind(data, predictions_12AM, predictions_1AM, predictions_2AM, predictions_3AM, predictions_4AM, predictions_5AM, predictions_6AM, predictions_7AM, predictions_8AM, predictions_9AM, predictions_10AM, predictions_11AM, predictions_12PM, predictions_1PM, predictions_2PM, predictions_3PM, predictions_4PM, predictions_5PM, predictions_6PM, predictions_7PM, predictions_8PM, predictions_9PM, predictions_10PM, predictions_11PM)
```


Now let's format the feglm_predict_data
```{r}
feglm_predict_data <- pivot_longer(feglm_predict_data, cols = starts_with("predictions_"),
                                  names_to = "Hour", 
                                  names_prefix = "predictions_",
                                  values_to = "Prediction")

#Turn the hour into a factor
feglm_predict_data$Hour <- factor(feglm_predict_data$Hour, ordered = TRUE, levels = c("12AM", "1AM", "2AM", "3AM", "4AM", "5AM", "6AM", "7AM", "8AM", "9AM", "10AM", "11AM", "12PM", "1PM", "2PM", "3PM", "4PM", "5PM", "6PM", "7PM", "8PM", "9PM", "10PM", "11PM"))

#Now we relabel
levels(feglm_predict_data$Hour) <- c(0:23)
```
...
names(weather_stats)
names(feglm_predict_data)
```


Lmer_predict_data had 1,084,880 rows and once we merge, we now have 528,820 so half disappeared. When we get rid of NAs, we have 528,191 rows. When we dedup, we end up with 345,936 rows
```{r}
#Deduplicate the weather stats data set
weather_stats_uniq <- weather_stats %>%
  arrange(STATION, DATE, Hour) %>%
  group_by(STATION, DATE, Hour) %>%
  slice_head(n = 1) %>%
  ungroup()

final_frame <- merge(feglm_predict_data, weather_stats_uniq, by.x = c("STATION", "Date", "Hour"), by.y = c("STATION", "DATE", "Hour"), all.x = TRUE)
necessary_names <- c(names(feglm_predict_data), "HourlyRelativeHumidity") 

table(duplicated(feglm_predict_data[, c("STATION", "Date", "Hour")]))
table(duplicated(weather_stats[, c("STATION", "DATE", "Hour")]))
final_frame <- final_frame[necessary_names]
final_frame <- final_frame[!is.na(final_frame$HourlyRelativeHumidity),]
#Now we export 
library(writexl)
#write_xlsx(final_frame, "/Volumes/Seagate Portable Drive/Relative_Humidity_Project/5.modeling_data/10_5_2025_Data/RH_Prediction_Data.xlsx")
```
...
The moment of truth
```{r}
cor(final_frame$Prediction, final_frame$HourlyRelativeHumidity)^2
