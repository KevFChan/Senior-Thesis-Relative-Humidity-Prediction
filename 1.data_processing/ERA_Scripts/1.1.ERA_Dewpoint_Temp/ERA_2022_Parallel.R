#This is a file to test out the ERA5 Data Cleaning for 2022
library(raster)
library(sf)
library(foreach)
library(doParallel)
...
process_era_5 <- function(raster_hour_data, year){
  ...
  #Rename the data
  names(raster_data_df) <- c("Longitude", "Latitude", "Temperature")
...
print(paste0("Currently processing: ", temp_files[4]))
raster_file <- temp_files[4]
...
string_dir <- paste0("/home/kfc27/project/ERAProcessingData/StepOneDPandTemp/ERA5Temp/era5", date_string, ".csv")
