...
The purpose of this Rmd is to take the EVI raster data and convert it into CSVs.
...
#Define the pattern, we will do EVI first
pattern_evi <- "EVI"

#Get the tif files
tif_files_evi <- list.files(path = raster_dir, pattern = pattern_evi, full.names = TRUE)
```
...
process_evi_tif <- function(file_name){
  ...
  #Now we want to extract the values and bound the results together. Note that we need to apply the scaling factor
  raster_values <- data.frame(raster::values(raster_data) * 0.0001)
  raster_coords <- data.frame(raster::coordinates(raster_data))
  
  raster_data <- cbind(raster_coords, raster_values)
  
  #Rename the data
  names(raster_data) <- c("Longitude", "Latitude", pattern_evi)
  #Get the date
...
Now let's rbind
```{r}
evi_data <- rbind(tif_2019, tif_2020, tif_2021, tif_2022, tif_2023)

#Let's fix the missing data code
evi_data <- evi_data %>%
  mutate(EVI = ifelse(EVI == -0.03, NA, EVI))
```
Now we save the file
```{r}
saveRDS(evi_data, "/Volumes/Seagate Portable Drive/3.clean_data/EVI_Data.RDS")

```

Need to change one thing about 
```{r}
