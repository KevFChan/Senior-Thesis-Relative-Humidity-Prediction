...
The purpose of this Rmd file is to take the NDVI raster data and convert it into CSV file format
...
#Define the pattern, we will do NDVI first
pattern_ndvi <- "NDVI"

#Get the tif files
tif_files_ndvi <- list.files(path = raster_dir, pattern = pattern_ndvi, full.names = TRUE)
```
...
process_ndvi_tif <- function(file_name){
  ...
  #Rename the data
  names(raster_data) <- c("Longitude", "Latitude", pattern_ndvi)
  #Get the date
...
Now let's rbind
```{r}
ndvi_data <- rbind(tif_2019, tif_2020, tif_2021, tif_2022, tif_2023)

#Let's fix the missing data code
ndvi_data <- ndvi_data %>%
  mutate(NDVI = ifelse(NDVI == -0.03, NA, NDVI))

summary(ndvi_data$NDVI)
```


Now we save
```{r}
saveRDS(ndvi_data, "/Volumes/Seagate Portable Drive/3.clean_data/NDVI_Data.RDS")
